...
- Fonts are loaded via `next/font/google` (Geist), so the first dev/build compile fetches from Google Fonts and requires outbound network. If that egress is blocked, switch to a system font stack in `src/app/layout.tsx`.
- Unit tests (`pnpm test`) cover pure logic only (e.g. `scoreQuiz` in `src/lib/quiz.ts`). The interactive quiz flow is best verified in the browser at `/quiz`.
